...
then
    ...
    exec "$@" --model $MODEL_NAME --host $HOST --port $PORT --tensor-parallel-size $RUNPOD_GPU_COUNT $EXTRA_VLLM_ARGS
else
    # If it is not, run the command without the --tensor-parallel-size flag
    exec "$@" --model $MODEL_NAME --host $HOST --port $PORT $EXTRA_VLLM_ARGS
fi
